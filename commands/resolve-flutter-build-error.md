---
name: resolve-flutter-build-error
description: Diagnose and fix Flutter build/runtime errors across Dart, Android, iOS, and package dependency layers.
---

Resolve a build or runtime failure.

1. Follow `../skills/debug-flutter-issues/SKILL.md`.
2. Start from reproducible command and full logs.
3. Apply smallest safe fix.
4. Re-run failing command and impacted tests.
5. Document root cause and prevention notes.
6. Always report: reproduction command, root cause, and post-fix verification command output.

